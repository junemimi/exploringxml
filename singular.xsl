...
Single type pokemon: <xsl:value-of select="pokemon[type[position() = 1] and not(type[position() = 2])]" />:
<xsl:apply-templates select="count(pokemon[type[position() = 1] and not(type[position() = 2])])" />
</xsl:template>
...
<xsl:template match="/pokedex">
  ...
  Count: <xsl:value-of select="count(/pokedex/pokemon[count(type) = 1])" />
  <table border="1">
    <tr bgcolor="#9acd32">
      <th>Name</th>
    ...
    <xsl:apply-templates select="/pokedex/pokemon[count(type) = 1]" />
  </table>
  </body>
  </html>
</xsl:template>
